...
def eval_single_ckpt(model, test_loader, args, eval_output_dir, logger, epoch_id, dist_test=False, pc_dir=None):
    # load checkpoint
...
def repeat_eval_ckpt(model, test_loader, args, eval_output_dir, logger, ckpt_dir, dist_test=False, pc_dir=None):
...
def update_3d_map(key, val, threed_map_softa):
...
def update_bev_map(key, val, bev_map_softa):
