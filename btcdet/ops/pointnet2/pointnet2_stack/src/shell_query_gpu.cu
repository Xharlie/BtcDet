...
__global__ void shell_query_kernel_stack(int B, int M, float inner_radius, float outer_radius, int nsample, \
    ...
    // :param new_xyz: (M1 + M2 ..., 3) centers of the shell query
...
void shell_query_kernel_launcher_stack(int B, int M, float inner_radius, float outer_radius, int nsample,
    ...
    shell_query_kernel_stack<<<blocks, threads>>>(B, M, inner_radius, outer_radius, nsample, new_xyz, new_xyz_batch_cnt, xyz, xyz_batch_cnt, idx);
