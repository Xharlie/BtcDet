...
class StackSAModuleMSG(nn.Module):
    ...
    def forward(self, xyz, xyz_batch_cnt, new_xyz, new_xyz_batch_cnt, features=None, empty_voxel_set_zeros=True, rotateMatrix=None, xyscales=None, zscales=None, vis=False):
        """
        ...
        for k in range(len(self.groupers)):
            result_lst = self.groupers[k](
                xyz, xyz_batch_cnt, new_xyz, new_xyz_batch_cnt, features, rotateMatrix=rotateMatrix, xyscales=xyscales, zscales=zscales
            )  # (M1 + M2, C, nsample)
            if rotateMatrix is not None:
                new_features, ball_idxs, prerot_xyz = result_lst
            else:
                new_features, ball_idxs = result_lst

            new_features = new_features.permute(1, 0, 2).unsqueeze(dim=0)  # (1, C, M1 + M2 ..., nsample)
            if vis:
                xyz_shift = torch.split(new_features[0].permute(1, 2, 0)[..., :3], new_xyz_batch_cnt.tolist()) # [M1, nsample, 3], [M2, nsample, 3]
                points_lst.append(xyz_shift[0])
                if prerot_xyz is not None:
                    pre_xyz_shift = torch.split(prerot_xyz.permute(0, 2, 1), new_xyz_batch_cnt.tolist())  # [M1, nsample, 3], [M2, nsample, 3]
                    prerot_points_lst.append(pre_xyz_shift[0])
        ...
        if vis:
            return new_xyz, new_features, [points_lst, prerot_points_lst]
        else:
